 ...
 * 
 */
public class MySQLReader extends AbstractScriptReader {

	static {
		ReaderManager.registerReader("mysql", MySQLReader.class);
	}
	...
	protected TableNode populateTableNode(String scriptFrag) {
		...
		while (offset > 0) {
			...
			endIndex = scriptFrag.indexOf(")", offset);
			String key = scriptFrag.substring(offset, endIndex).replace('`',' ').replace(" ", "");
